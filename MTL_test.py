...
#  read frames from video 
def read_frame_from_folder(folder_name):
    frames = []
    filename_list = os.listdir(folder_name)
    for filename in sorted(filename_list):

        image = Image.open(folder_name + filename)
        # image = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
        frames.append(image.resize((w,h)))
    return frames       
...
def main_worker():
    ...
    frames = read_frame_from_folder(args.folder)
    ...
    print('loading videos and masks from: {}'.format(args.folder))
    ...
    writer = cv2.VideoWriter(f"{args.result}_result.mp4", cv2.VideoWriter_fourcc(*"mp4v"), default_fps, (w, h))
